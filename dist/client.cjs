...
var AuthConfiguration = class _AuthConfiguration {
  /**
  ...
  static load(config) {
    ...
    _AuthConfiguration.#checkMandatoryVariables(config, ["authType"]);
    switch (config.authType) {
      case AuthType.USER_SUPPLIED:
        config = _AuthConfiguration.#loadUserSuppliedAuth(config);
        break;
      case AuthType.USERNAME_PASSWORD:
        _AuthConfiguration.#checkMandatoryVariables(config, [
          "loginUrl",
          "username",
          "password"
        ]);
      ...
      case AuthType.OAUTH_JWT_BEARER:
        ...
        break;
      default:
        throw new Error(`Unsupported authType value: ${config.authType}`);
    }
    return config;
  }
  /**
  ...
  static #loadUserSuppliedAuth(config) {
    ...
    if (!config.organizationId) {
      try {
        ...
        throw new Error("Unable to parse organizationId from access token", {
          cause: error
        });
      }
    }
...
function parseEvent(schema, event, topicName) {
...
var SalesforceAuth = class {
  ...
  async authenticate() {
    ...
    switch (this.#config.authType) {
      ...
      default:
        throw new Error(`Unsupported authType value: ${this.#config.authType}`);
    }
  }
  /**
...
var PubSubApiClient = class {
  ...
  async connect() {
    ...
    try {
      ...
      const protoFilePath = (0, import_url.fileURLToPath)(
        new URL("./pubsub_map-4754b3f9.proto?hash=4754b3f9", "file://" + __filename)
      );
  ...
  #subscribe(subscribeRequest, subscribeCallback) {
    ...
    try {
      ...
      if (subscription) {
        this.#logger.debug(`${topicName} - Reusing cached gRPC subscription`);
        ...
        this.#logger.debug(`${topicName} - Establishing new gRPC subscription`);
